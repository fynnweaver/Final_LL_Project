...
centers, sample_clusters, cluster_distances = cluster(dataset.features(), thres=args.thres, min_clus=args.min_clus,
                                                max_dist=args.max_dist, normalize=args.normalize)
end_time = time.time()

# Save samples into their corresponding clusters
dataset.save_clustered_samples(sample_clusters, centers)
