...
class BaseDataset:
    ...
    def save_clustered_samples(self, sample_clusters, centers):
    
        dest = os.path.join(self.data_dir, 'clusters')
        shutil.rmtree(os.path.join(dest), ignore_errors=True)
        os.mkdir(dest)
        #np.save(os.path.join(self.data_dir, 'cluster_idx.npy'), sample_clusters)
        np.save(os.path.join(self.data_dir, 'centers.npy'), centers)
